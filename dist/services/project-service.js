...
const variable_substitution_service_1 = require("./variable-substitution.service");
class ProjectService {
    templateService;
    fileService;
    configService;
    variableService;
    constructor(templateService, fileService, configService) {
        this.templateService = templateService;
        this.fileService = fileService;
        this.configService = configService;
        this.variableService = new variable_substitution_service_1.VariableSubstitutionService(this.fileService);
    }
    async createProject(projectName, templateIds, targetPath, variables) {
    ...
    async fixProject(projectPath, dryRun) {
        if (!projectPath || typeof projectPath !== 'string') {
            throw new Error('Project path must be a non-empty string');
        }
        const startTime = Date.now();
                        ...
                        if (error.fix?.autoFix) {
                            ...
                            if (error.fix.action === 'create') {
                                if (error.ruleId === 'required_folder') {
                                    ...
                                    const file = template.files.find(f => this.variableService.substituteInPath(f.path, manifest.variables) === error.path);
    ...
    async extendProject(projectPath, templateIds, variables) {
        if (!projectPath || typeof projectPath !== 'string') {
            throw new Error('Project path must be a non-empty string');
        }
        if (!Array.isArray(templateIds) || templateIds.length === 0) {
    ...
    async loadProjectManifest(projectPath) {
        return this.getProjectManifest(projectPath);
    }
    async saveProjectManifest(projectPath, manifest) {
        await this.updateProjectManifest(projectPath, manifest);
    }
    async cleanProject(projectPath) {
        ...
        try {
            ...
            const shouldLogVerbose = this.configService?.get('preferences.verboseOutput') || false;
            if (shouldLogVerbose) {
     ...
     */
    async findNearestManifest(startPath) {
     ...
     */
    async getProjectManifest(projectPath) {
        if (!projectPath || typeof projectPath !== 'string') {
            throw new Error('Project path must be a non-empty string');
        }
        ...
        try {
            if (await this.fileService.exists(directManifestPath)) {
                const manifestData = await this.fileService.readJson(directManifestPath);
                return manifestData;
            }
            // If not found, search upward for the nearest manifest
            const nearestManifest = await this.findNearestManifest(projectPath);
            if (nearestManifest) {
                const manifestData = await this.fileService.readJson(nearestManifest.manifestPath);
                return manifestData;
            }
            return null;
        }
     ...
     */
    async updateProjectManifest(projectPath, manifest) {
        if (!projectPath || typeof projectPath !== 'string') {
            throw new Error('Project path must be a non-empty string');
        }
     ...
     */
    initializeProjectManifest(projectName, templateId) {
        if (!projectName || typeof projectName !== 'string') {
            throw new Error('Project name must be a non-empty string');
        }
     ...
     */
    async applyTemplate(template, projectPath, variables) {
        try {
